...
def cvmm(x: torch.Tensor, sel: Union[torch.Tensor, CVMMSel], keys: torch.Tensor):
    ...
    v_triton = version.parse(triton.__version__)
    if (v_triton >= version.parse("2.2.0")) and (v_triton < version.parse("3.0.0") and 
            torch.cuda.get_device_properties(0).major <= 7 and torch.is_autocast_enabled()):
        print("------------------------------- ERROR -------------------------------")
        print("ERROR: 2.2 <= Triton < 3.0 seems to be broken on Volta GPUs.")
        ...
        print("---------------------------------------------------------------------")
        raise RuntimeError("2.2 <= PyTorch < 2.4 Triton with AMP is to be broken on Volta GPUs.")
